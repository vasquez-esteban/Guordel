...
class BotonSeleccion:
    ...
    def __init__(self, x, y, escala, cant_opciones, *args):
        ...
        for x in range(cant_opciones):
            texto = args[x]
            ancho = (
                args[cant_opciones + 1]
                if len(args) > cant_opciones
                else len(texto) * 8 * escala + 8 / len(texto)
            )
            altura = (
                args[cant_opciones + 2] if len(args) > cant_opciones else 20 * escala
            )
            self.opciones[texto] = Opcion(
                inicio, y, ancho, altura, texto, Rect(inicio, y, ancho, altura)
            )
            inicio += ancho + 10
    ...
    def draw(self, surface, varObjetivo):
        ...
        for boton in self.opciones:
            ...
            color_recuadro = (
                objeto.color_de_boton
                if varObjetivo != boton
                else objeto.boton_presionado
            )
            ...
            surface.blit(
                imagen_texto,
                (
                    objeto.meta_x + objeto.ancho * (1 / 7),
                    objeto.meta_y + objeto.altura * (1 / 8),
                ),
            )
        return varObjetivo
