...
class Boton:
    ...
    # constructor -> args: se usa para
    # agregar un tamaño especifico al botón (ingresa dos valores para ancho y después largo)
    # constructor -> kwargs["imagen"]:
    # si en el constructor escribes imagen= el botón va a ser la imagen que le agregues al parametro
    ...
    def __init__(self, x, y, escala, *args, **kwargs):
        ...
        if "texto" in kwargs:
            self.ancho = (
                args[0]
                if args
                else len(kwargs["texto"]) * 8 * escala + 8 / len(kwargs["texto"])
            )
    ...
    def draw(self, surface):
        ...
        if self.texto:
            ...
            surface.blit(
                imagen_texto,
                (self.x + self.ancho * (1 / 7), self.y + self.altura * (1 / 8)),
            )
